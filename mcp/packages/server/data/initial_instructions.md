...
When the user asks to create a screen, widget, dashboard, or other UI:
  ...
  * Crie tokens primeiro (`tokens plan`/`tokens setup` via CLI ou ferramentas equivalentes), depois componentes, depois screens.
  * Use `create_screen_page` e `create_screen_shell` para scaffolding de screen antes de colocar instâncias reutilizáveis.
